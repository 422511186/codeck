...
当前自定义绑定的输入模态 SHALL 约束待发送草稿。切换到只支持文本的模型时，系统 MUST 保留已选图片并允许切换，但 MUST 阻止发送，直到用户移除图片或切换回支持图片的模型。历史消息中的图片 MUST NOT 阻止模型切换。

#### Scenario: Draft images survive text-only switch
- **WHEN** 草稿包含图片且用户切换到仅支持文本的模型
- **THEN** 切换 MUST 继续执行并保留图片与文本草稿
- **AND** 发送按钮 MUST 被阻止并显示输入不兼容状态

#### Scenario: Historical images do not block
- **WHEN** 会话历史包含图片但当前草稿与目标能力兼容
- **THEN** 历史图片 MUST NOT 阻止切换
- **AND** 后续真实 turn 的 provider 兼容错误 MUST 通过正常错误流程呈现
